...
object task_collections {
  ...
  def capitalizeIgnoringASCII(text: List[String]): List[String] = {
    text.zipWithIndex.collect {
      case (el, i) if i > 0 => if (isASCIIString(el)) el.toUpperCase else el.toLowerCase()
      case (el, _) => el
    }
  }
  ...
  def numbersToNumericString(text: String): String = {
    val numbers = Map("1" -> "one", "2" -> "two", "3" -> "three", "4" -> "four", "5" -> "five", "6" -> "six", "7" -> "seven", "8" -> "eight", "9" -> "nine")
   ...
   **/
  def intersectionAuto(dealerOne: Iterable[Auto], dealerTwo: Iterable[Auto]): Iterable[Auto] = (dealerOne ++ dealerTwo).toSet
